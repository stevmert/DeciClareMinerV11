...
public class AtMostLag extends RelationConstraint implements BoundedConstraint {

	private static final long serialVersionUID = 8001661256329490997L;

	private int atMost;

	public AtMostLag(Decision activationDecision, Decision deactivationDec, ExistenceExpression conditionExpression,
			ExistenceExpression consequenceExpression, int atMost, boolean isOptional) {
	...
	protected String getTextualConstraint() {
		return "before " + getConsequenceExpression().toTextualString() + " can be executed after the execution of "
				+ getConditionExpression().toTextualString() + " at most " + getTime(getBound()) + " have to have elapsed";
	}
	...
	public Constraint getDecisionlessCopy() {
		return new AtMostLag(null, null, getConditionExpression(), getConsequenceExpression(),
				getBound(), isOptional());
	}
	...
	public static Rule mineAtLeastLag(Decision activationDecision,
		...
		//			pos = new Rule(new AtMostLag(activationDecision, conditionExpression, consequenceExpression, timeMin, timeMax, true),
		...
		//			neg = new Rule(new AtMostLag(activationDecision, conditionExpression, consequenceExpression, 0, -1, false),
	...
	public RelationConstraint getShallowCopy(ExistenceExpression conditionExpression,
		...
		return new AtMostLag(getActivationDecision(), getDeactivationDecision(), conditionExpression,
